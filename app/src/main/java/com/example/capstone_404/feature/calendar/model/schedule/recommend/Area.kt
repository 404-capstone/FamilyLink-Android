...
object Areas {
    ...
    // 시·도별 군·구 리스트
    private val guListBySiDo: Map<String, List<String>> = mapOf(
        // 서울
        "서울특별시" to listOf(
    ...
    // "전체" 추가한 출력용 리스트
    fun guListWithAll(siDoFull: String): List<Area> {
        val listData = guListBySiDo[siDoFull].orEmpty()
        return listOf(Area(full = "전체", short = "전체")) +
                listData.map { gu -> Area(full = gu, short = gu) }
    }
}